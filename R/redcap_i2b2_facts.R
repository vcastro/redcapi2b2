...

{
  ...
  convert_to_POSIXct <- function(date_string) {
    date <- tryCatch({
      if (grepl(":", date_string)) {
        # If the string contains a time component
        if (nchar(date_string) > 16) {
          # If the string contains seconds
          as.POSIXct(date_string, format = "%m/%d/%Y %H:%M:%S")
        } else {
          # If the string contains only hours and minutes
          as.POSIXct(date_string, format = "%m/%d/%Y %H:%M")
        }
      } else {
        # If the string is just a date
        as.POSIXct(date_string, format = "%m/%d/%Y")
      }
    }, error = function(e) {
      NA
    })
  ...
  form_date <- function(redcap_data, form_name, date_fields, redcap_default_fields) {
    ...
    redcap_data |>
      ...
      dplyr::select(tidyselect::any_of(c(redcap_default_fields, "form_name", "start_date")))
  ...
  date_fields <- purrr::imap(date_mappings,
  ...
  redcap_data_long_mapped <- redcap_data_long |>
    ...
    dplyr::left_join(date_fields, by = c(redcap_default_fields, "i_form_name"="form_name")) |>
    dplyr::mutate(
      i2b2_field_type = dplyr::case_when(
        .data$i_field_type %in% c("radio", "checkbox", "dropdown", "yesno") ~ "choice",
        ...
        .data$i_field_type %in% c("text", "notes") &
          .data$df_data_type == "character" & stringr::str_length(.data$value) <= 255 ~ "text",
        .data$i_field_type %in% c("notes") & stringr::str_length(.data$value) > 255 ~ "blob",
  ...
  observation_fact <- redcap_data_long_mapped |>
    dplyr::transmute(
      ...
      ENCOUNTER_IDE = ifelse("redcap_event_name" %in% names(redcap_data),
      ...
      END_DATE = NA,
      PROVIDER_ID = ifelse("redcap_data_access_group" %in% names(redcap_data),
                           .data$redcap_data_access_group,
                          "@"),
      MODIFIER_CD = "@",
      INSTANCE_NUM = ifelse("redcap_repeat_instance" %in% names(redcap_data),
                            dplyr::coalesce(.data$redcap_repeat_instance, 1),
                            1),
      VALTYPE_CD = dplyr::case_when(.data$i2b2_field_type == "numeric" ~ "N",
                             .data$i2b2_field_type == "text" ~ "T",
                             .data$i2b2_field_type == "blob" ~ "B",
                             .data$i2b2_field_type == "data" ~ "T",
                             TRUE ~ NA_character_),
      NVAL_NUM = ifelse(.data$i2b2_field_type == "numeric",
                        ifelse(is.numeric(.data$value), as.numeric(.data$value), NA_real_),
                        NA_real_),
      TVAL_CHAR = dplyr::case_when(.data$i2b2_field_type == "text" ~ .data$value,
      ...
      OBSERVATION_BLOB = ifelse(.data$i2b2_field_type == "blob", .data$value, NA_character_),
